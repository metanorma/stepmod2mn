...
<!--
$Id: sect_e_exp_arm_lf.xsl,v 1.6 2003/10/01 07:49:01 robbod Exp $
...
<xsl:template match="module">
  ...
  (*<br/>
    <xsl:value-of 
      select="concat('ISO/TC 184/SC 4/WG ',$wg_group,'&#160;N',@wg.number.arm_lf, ' - ',
              $stdnumber,' ', $module_name, ' - EXPRESS ARM Long form')"/>
    <xsl:if test="string-length(normalize-space(@wg.number.arm_lf.supersedes))>0">
      <br/>Supersedes 
      <xsl:value-of 
        select="concat('ISO/TC 184/SC 4/WG ',$wg_group,'&#160;N',@wg.number.arm_lf.supersedes)"/>
    </xsl:if>
    <br/>*)
  </code>
  <br/>
