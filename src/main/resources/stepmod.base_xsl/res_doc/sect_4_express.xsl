...
<!--  <xsl:import href="express_link.xsl"/> 
  ...
  <!--<xsl:variable name="note_number" select="count(document(concat('../../data/resource_docs/', $resdoc_name, '/resource.xml'))//introduction//note)"/> MWD 2018-02-06 -->
...
<!--     </code> -->
  ...
  <xsl:if test="position()=last()">
    <xsl:call-template name="interface_notes">
      ...
      <xsl:with-param name="schema_name_param" select="$schema_name"/><!-- MWD 2018-02-06 -->
    </xsl:call-template>
  </xsl:if>
</xsl:template>
      ...
      <xsl:choose>
        <xsl:when test="$resource_ok='true'">
          ...
          <xsl:variable name="reference">
            <xsl:value-of
              select="document(concat($path, '../../../data/resources/',$resource,'/',$resource,'.xml'))/express/@reference"/>
          </xsl:variable>
...
<xsl:template match="rule">
 ...
 <!-- <p> -->
  <!-- start blockquote -->
    ...
    <br/>
      (<xsl:value-of select="translate(@appliesto,' ',', ')"/>);<xsl:text> +&#xa;</xsl:text><!-- <br/> -->
		<xsl:call-template name="insertCodeEnd"/>
  <!-- </code> -->
    <xsl:apply-templates select="./algorithm" mode="code"/>
