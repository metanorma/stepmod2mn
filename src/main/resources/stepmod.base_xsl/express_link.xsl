...
<xsl:template name="link_list">
  ...
  <xsl:param name="clause" select="section"/>
  <xsl:param name="and_for_last_pair" select="'no'"/>
  <!--<xsl:param name="initial_pair_param" select="yes"/>-->
 
  <xsl:variable name="nlist"
    select="translate(normalize-space($list),' ',',')"/>
  
  <!--<xsl:variable name="initial_pair">
    <xsl:choose>
      <xsl:when test="$initial_pair_param='yes'">
        <xsl:choose>
          <xsl:when test="contains(substring-after($nlist, ','), ',')">
  ...
  <xsl:choose>
	...
	<xsl:when test="$and_for_last_pair='yes' and
	  ...
	  <xsl:value-of select="' and '"/><!-- MWD 2017-07-25 comma removed 2019-05-31 -->
          <xsl:choose>
            <xsl:when test="$bold='yes'">
              <b>
                <xsl:call-template name="link_object">
